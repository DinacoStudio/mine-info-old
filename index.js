...
class MineInfo {
    ...
    async getInfo(address, port) {
        ...
        for (let api_object of this.#api) {
            ;(await this.#isAlive(api_object.host).catch(e => console.log(e)))
                .alive
                ? (this.#aliveApi[api_object.sample] = api_object)
                : null
        }
        this.#ping_log && console.timeEnd(colors.green('ping'))

        if (!Object.keys(this.#aliveApi).length)
            throw new Error('Ошибка нету доступа ни к одной api')
         ...
         * Дохуя API
         */
        this.#aliveApi['api.mcsrvstat.us'] &&
            (response.mcsrvstat = (
                await this.#get(
                    `${this.#aliveApi['api.mcsrvstat.us'].api}${address}${
                        port ? `:${port}` : ''
                    }`
                ).catch(e => {
                    console.warn(
                        colors.red(
                            'Невозможно получить данные с api.mcsrvstat.us'
                        )
                    )
                    this.#logging &&
        ...
        this.#aliveApi['eu.mc-api.net'] &&
            (response.mcapi = (
                await this.#get(
                    `${this.#aliveApi['eu.mc-api.net'].api}${address}${
                        port ? `:${port}` : ''
                    }`
                ).catch(e => {
                    console.warn(
                        colors.red('Невозможно получить данные с eu.mc-api.net')
                    )
                    this.#logging &&
                        fs.appendFileSync('log.txt', e.toString() + '\n')
                })
            )?.body)
        this.#aliveApi['mcapi.us'] &&
            (response.mcapius = (
                await this.#get(
                    `${this.#aliveApi['mcapi.us'].api}${address}${
                        port ? `&${port}` : ''
                    }`
                ).catch(e => {
                    console.warn(
                        colors.red('Невозможно получить данные с mcapi.us')
                    )
                    this.#logging &&
                        fs.appendFileSync('log.txt', e.toString() + '\n')
                })
            )?.body)
        this.#aliveApi['api.mcstatus.io'] &&
            (response.mcstatus = (
                await this.#get(
                    `${this.#aliveApi['api.mcstatus.io'].api}${address}${
                        port ? `:${port}` : ''
                    }`
                ).catch(e => {
                    console.warn(
                        colors.red(
                            'Невозможно получить данные с api.mcstatus.io'
                        )
                    )
                    this.#logging &&
                        fs.appendFileSync('log.txt', e.toString() + '\n')
        ...
        this.#aliveApi['api.minetools.eu'] &&
            (response.minetools = (
                await this.#get(
                    `${this.#aliveApi['api.minetools.eu'].api}${address}${
                        port ? `/${port}` : ''
                    }`
                ).catch(e => {
                    console.warn(
                        colors.red(
                            'Невозможно получить данные с api.minetools.eu'
                        )
                    )
                    this.#logging &&
                        fs.appendFileSync('log.txt', e.toString() + '\n')
        ...
        this.#aliveApi['mcstatus.snowdev.com.br'] &&
            (response.snowdev = (
                await this.#get(
                    `${
                        this.#aliveApi['mcstatus.snowdev.com.br'].api
                    }${address}${port ? `:${port}` : ''}`
                ).catch(e => {
                    console.warn(
                        colors.red(
                            'Невозможно получить данные с mcstatus.snowdev.com.br'
                        )
                    )
                    this.#logging &&
                        fs.appendFileSync('log.txt', e.toString() + '\n')
        ...
        this.#aliveApi['mcapi.xdefcon.com'] &&
            (response.xdefcon = (
                await this.#get(
                    `${this.#aliveApi['mcapi.xdefcon.com'].api}${address}${
                        port ? `:${port}` : ''
                    }/full/json`
                ).catch(e => {
                    console.warn(
                        colors.red(
                            'Невозможно получить данные с mcapi.xdefcon.com'
                        )
                    )
                    this.#logging &&
                        fs.appendFileSync('log.txt', e.toString() + '\n')
        ...
        let temp_port = this.#Truphy([
            response?.mcapi?.dns?.port,
            response?.mcstatus?.port,
            response?.mcsrvstat?.port,
        ])
        this.#aliveApi['dinaco.ds1nc.ru'] &&
            (response.mine = (
                await this.#get(
                    `${this.#aliveApi['dinaco.ds1nc.ru'].api}${address}${
                        temp_port ? `&port=${temp_port}` : ''
                    }`
                ).catch(e => {
                    console.warn(
                        colors.red(
                            'Невозможно получить данные с dinaco.ds1nc.ru'
                        )
                    )
                    this.#logging &&
                        fs.appendFileSync('log.txt', e.toString() + '\n')
        ...
        const motd = this.#Truphy([
            ...
            xdefcon?.motd?.text,
            mcapius?.motd,
            mine?.motd?.clean?.extra?.reduce((prev, next) => {
                return { text: prev.text + next.text }
            }).text,
        ])
        ...
        const hostInfo = {
            ip: this.#Truphy([mcsrvstat?.ip, mcapi?.dns?.ip, mcstatus?.host]),
            port: this.#Truphy([
                mcapi?.dns?.port,
                mcstatus?.port,
                mcsrvstat?.port,
        ...
        const modInfo = {
            type: this.#Truphy([mcapi?.modinfo?.type, null]),
            mods: this.#Truphy([mcapi?.modinfo?.modList, mcstatus?.mods, null]),
        }
